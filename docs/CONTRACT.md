...
Native requests default to at most 256 questions, 60,000 serialized UTF-8 bytes, and 30,000 bytes for state plus the longest question. Host `capacity.json` can lower byte caps and configure question count up to 4,096; size caps still bind. These are conservative byte heuristics, not a provider tokenizer. Individual recipe text is capped at 6,000 UTF-8 bytes; raw batch text at 12,000. Choice supports 2–255 options within the same size limits. Shared state is visible to all questions; per-item prompt separation is not confidentiality isolation. Dependent questions require separate steps. [Bulk contract](BULK.md).
...
New v0.4 calls share atomic admission limits per provider route and ledger: default four concurrent pending calls, 1,200 starts per rolling minute and 200,000 serialized request bytes per rolling second. Hosts can configure these in `capacity.json`; they do not establish provider entitlements. Provider rate limits still apply, and byte admission is not exact token-rate metering. Legacy calls have no admission rows and only their shared accounting/uncertainty limits apply during migration: stop old workers before relying on the new concurrency limits. Refusals reserve nothing. A bulk run returns progress/backpressure instead of internally retrying. The fixed transport disables redirects and inherited proxies, caps response bytes at 4 MiB and runs under a 20-second wall deadline. No arbitrary endpoint argument or silent provider fallback is exposed.

Settled same-ID responses remain readable when a valid policy is disabled or the key is absent. New admission still requires both. The coarse ready state permits ordinary active calls but refuses stale uncertain ones; readiness never guarantees remaining budget or capacity.
